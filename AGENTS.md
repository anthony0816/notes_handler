...
| `modules/utils/todo.py` | Helpers compartidas: `notes_root`, `todo_path`, `sub_path`, `current_path`, `current_sub`, `read_lines`, `write_lines`, `parse_task`, `todo_items`, `filter_task`, `mode_from_args`, constantes `TASK_RE`, `DEFAULT_TODO_REL`, `ENV`. |
...
- Para el agente: git SOLO en modo lectura (`git status`, `git log`, `git
  diff`). Nunca ejecutar operaciones que escriban o deshagan cambios
  (`reset`, `checkout`, `clean`, `restore`, `commit`, `push`); eso lo pide el
  usuario explícitamente via `todo sync` / `todo restore`.
